                    ...
                    <nav aria-label="breadcrumb">
                        <ol class="breadcrumb">
                            <li class="breadcrumb-item"><a href="{{ route('admin.dashboard') }}"
                                    class="text-decoration-none" style="color: var(--btn-bg);">{{ __('Dashboard') }}</a>
                            </li>
                            <li class="breadcrumb-item"><a href="{{ route('admin.users.index') }}"
                                    class="text-decoration-none" style="color: var(--btn-bg);">{{ __('Utilisateurs') }}</a>
                            </li>
                            <li class="breadcrumb-item active" aria-current="page" style="color: var(--text-main);">
                                {{ $user->name }}</li>
                        </ol>
                    </nav>
                </div>
            ...
            <div class="row g-4">
                ...
                <div class="col-xl-4">
                    <div class="card card-custom border-0 shadow-sm text-center p-4">
                        <div class="rounded-circle d-flex justify-content-center align-items-center fw-bold text-white shadow-sm mx-auto mb-3"
                            style="width: 100px; height: 100px; font-size: 2.5rem; background-color: var(--btn-bg);">
                        ...
                        <div class="d-flex justify-content-center gap-2 mb-4">
                            ...
                            @endif
                            @if($user->is_blocked)
                                <span class="badge bg-danger px-3 py-2 rounded-pill"><i class="fas fa-lock me-1"></i>
                                    {{ __('Bloqué') }}</span>
                            @else
                                <span class="badge bg-success px-3 py-2 rounded-pill"><i class="fas fa-check-circle me-1"></i>
                                    {{ __('Actif') }}</span>
                            @endif
                        ...
                        <ul class="list-group list-group-flush text-start mb-4">
                            <li
                                ...
                                <span class="opacity-75"><i class="fas fa-calendar-alt me-2"></i>
                                    {{ __('Inscrit le') }}</span>
                        ...
                        @if(auth()->id() !== $user->id)
                            <form action="{{ route('admin.users.toggle-block', $user->id) }}" method="POST"
                                ...
                                @method('PATCH')
                                <button type="submit"
                                    class="btn w-100 rounded-pill fw-bold py-2 {{ $user->is_blocked ? 'btn-success' : 'btn-danger' }}">
                ...
                <div class="col-xl-8">
                    <div class="card card-custom border-0 shadow-sm p-4 h-100">
                        <h4 class="fw-bold mb-4" style="color: var(--text-main);"><i class="fas fa-history me-2"
                                style="color: var(--btn-bg);"></i>{{ __('Dernières recettes proposées') }}</h4>

                        @if($user->recipes->count() > 0)
                            <div class="list-group list-group-flush">
                                @foreach($user->recipes as $recipe)
                                    <div
                                        class="list-group-item bg-transparent px-0 py-3 border-color d-flex align-items-center justify-content-between">
                                        <div class="d-flex align-items-center gap-3">
                                            <div class="rounded-3"
                                                style="width: 60px; height: 60px; background: url('{{ asset($recipe->image_url ?? 'images/default-recipe.jpg') }}') center/cover;">
                                            </div>
                                            <div>
                                                <h6 class="mb-1 fw-bold" style="color: var(--text-main);">{{ $recipe->name }}</h6>
                                                <div class="small opacity-75">
                                                    <span class="me-3"><i class="far fa-clock me-1"></i>
                                                        {{ $recipe->created_at->diffForHumans() }}</span>
                                                    @if($recipe->status === 'approved')
                                                        <span class="text-success"><i class="fas fa-check-circle me-1"></i>
                                                            Approuvée</span>
                                                    @elseif($recipe->status === 'pending')
                                                        <span class="text-warning"><i class="fas fa-hourglass-half me-1"></i> En
                                                            attente</span>
                                                    @else
