                                ...
                                <div class="border-top border-color bg-soft">
                                    <div class="comments-list custom-scrollbar px-3 pt-3" id="commentsList{{ $recipe->id }}" style="max-height: 200px; overflow-y: auto;">
                                        ...
                                        @forelse($topCardComments as $comment)
                                            <div class="mb-2 p-2 rounded-3 bg-body-tertiary shadow-xs" style="font-size:0.8rem;" id="comment-{{ $comment->id }}">
                                                <div class="d-flex justify-content-between mb-1">
                                                    ...
                                                    <div class="d-flex align-items-center gap-2">
                                                        ...
                                                        @auth
                                                            <button class="btn btn-link p-0 border-0 reply-btn" style="font-size:0.65rem;color:var(--btn-bg);" data-comment-id="{{ $comment->id }}" data-recipe-id="{{ $recipe->id }}" data-recipe-url="{{ route('comments.store', $recipe->id) }}"><i class="fas fa-reply"></i></button>
                                                        @endauth
                                                        @if(Auth::check() && Auth::id() === $comment->user_id)
                                                            <form action="{{ route('comments.destroy', $comment->id) }}" method="POST" class="d-inline" onsubmit="return confirm('{{ __('Supprimer ?') }}')">@csrf @method('DELETE')<button type="submit" class="btn btn-link text-danger p-0 border-0" style="font-size:0.65rem;"><i class="fas fa-trash-alt"></i></button></form>
                                                        @endif
                                                    </div>
                                                </div>
                                                ...
                                                <div class="reply-form-container d-none" id="reply-form-{{ $comment->id }}">
                                                    <form class="comment-form d-flex gap-1 mt-1" data-recipe-id="{{ $recipe->id }}" data-url="{{ route('comments.store', $recipe->id) }}" data-parent-id="{{ $comment->id }}">
                                                        <input type="hidden" name="parent_id" value="{{ $comment->id }}">
                                                        <input type="text" name="body" class="form-control form-control-sm border-color bg-body shadow-none rounded-pill px-2" style="font-size:0.75rem;" placeholder="{{ __('Répondre...') }}" required autocomplete="off">
                                                        <button type="submit" class="btn btn-main btn-sm rounded-circle d-flex align-items-center justify-content-center" style="width:26px;height:26px;"><i class="fas fa-paper-plane" style="font-size:0.6rem;"></i></button>
                                                    </form>
                                                </div>
                                                @endauth
                                                {{-- Replies --}}
                                                <div class="mt-1 ps-2 border-start border-color replies-list-{{ $comment->id }}">
                                                    @if($comment->replies->count() > 0)
                                                        @foreach($comment->replies->sortBy('created_at')->take(2) as $reply)
                                                            <div class="d-flex justify-content-between align-items-center gap-1 mb-1" style="font-size:0.75rem;" id="comment-{{ $reply->id }}">
                                                                <div>
                                                                    <span class="fw-bold text-main">{{ $reply->user->name ?? '?' }}:</span>
                                                                    <span class="opacity-75">{{ Str::limit($reply->body, 60) }}</span>
                                                                </div>
                                                                @if(Auth::check() && Auth::id() === $reply->user_id)
                                                                    <form action="{{ route('comments.destroy', $reply->id) }}" method="POST" class="d-inline" onsubmit="return confirm('{{ __('Supprimer ?') }}')">
                                                                        @csrf @method('DELETE')
                                                                        <button type="submit" class="btn btn-link text-danger p-0 border-0" style="font-size:0.65rem;"><i class="fas fa-trash-alt"></i></button>
                                                                    </form>
                                                                @endif
                                                            </div>
                                                        @endforeach
                                                        @if($comment->replies->count() > 2)
                                                            <span class="opacity-50" style="font-size:0.7rem;">+{{ $comment->replies->count() - 2 }} {{ __('réponses') }}</span>
                                                        @endif
                                                    </div>
                                                @endif
                                            </div>
                        ...
                        <div class="row g-0">
                            ...
                            <div class="col-md-7 p-4">
                                ...
                                <div class="border-top border-color pt-4">
                                    ...
                                    @endauth
                                    <div class="modal-comments-list custom-scrollbar" id="modalCommentsList{{ $recipe->id }}" style="max-height:350px;overflow-y:auto;">
                                        @php $topComments = $recipe->comments->whereNull('parent_id')->sortByDesc('created_at'); @endphp
                                        @forelse($topComments as $comment)
                                            <div class="mb-3 p-3 rounded-4 bg-soft comment-thread" id="modal-comment-{{ $comment->id }}">
                                                <div class="d-flex align-items-start gap-2">
                                                    <div class="rounded-circle d-flex align-items-center justify-content-center text-white fw-bold flex-shrink-0" style="width:30px;height:30px;font-size:0.75rem;background:linear-gradient(135deg,var(--btn-bg),var(--btn-hover));">{{ strtoupper(substr($comment->user->name ?? '?', 0, 1)) }}</div>
                                                    <div class="flex-grow-1">
                                                        <div class="d-flex justify-content-between align-items-center flex-wrap gap-1 mb-1">
                                                            <span class="fw-bold text-main" style="font-size:0.85rem;">{{ $comment->user->name ?? '?' }}</span>
                                                            <div class="d-flex align-items-center gap-2">
                                                                <span class="small opacity-50" style="font-size:0.75rem;">{{ $comment->created_at->diffForHumans() }}</span>
                                                                @auth
                                                                    <button class="btn btn-link p-0 border-0 reply-btn" style="font-size:0.75rem;color:var(--btn-bg);" data-comment-id="{{ $comment->id }}" data-recipe-id="{{ $recipe->id }}" data-recipe-url="{{ route('comments.store', $recipe->id) }}"><i class="fas fa-reply me-1"></i>{{ __('Répondre') }}</button>
                                                                @endauth
                                                                @if(Auth::check() && Auth::id() === $comment->user_id)
                                                                    <form action="{{ route('comments.destroy', $comment->id) }}" method="POST" class="d-inline" onsubmit="return confirm('{{ __('Supprimer ?') }}')">@csrf @method('DELETE')<button type="submit" class="btn btn-link text-danger p-0 border-0" style="font-size:0.75rem;"><i class="fas fa-trash-alt"></i></button></form>
                                                                @endif
                                                            </div>
                                                        </div>
                                                        <p class="mb-0 text-main opacity-75" style="font-size:0.85rem;">{{ $comment->body }}</p>
                                                    </div>
                                                </div>
                                                @auth
                                                <div class="reply-form-container ms-4 mt-2 d-none" id="reply-form-{{ $comment->id }}">
                                                    <form class="comment-form d-flex gap-2" data-recipe-id="{{ $recipe->id }}" data-url="{{ route('comments.store', $recipe->id) }}" data-parent-id="{{ $comment->id }}">
                                                        <input type="hidden" name="parent_id" value="{{ $comment->id }}">
                                                ...
                                                @endauth
                                                <div class="ms-4 mt-2 replies-container replies-list-{{ $comment->id }}" id="replies-{{ $comment->id }}">
                                                    @if($comment->replies->count() > 0)
                                                        @if($comment->replies->count() > 2)
                                                            <button class="btn btn-link p-0 border-0 mb-2 toggle-replies-btn" style="font-size:0.75rem;color:var(--btn-bg);" data-target="replies-inner-{{ $comment->id }}"><i class="fas fa-chevron-down me-1"></i>{{ __('Voir') }} {{ $comment->replies->count() }} {{ __('réponses') }}</button>
                                                            <div id="replies-inner-{{ $comment->id }}" class="d-none">
                                                        @endif
                                                        @foreach($comment->replies->sortBy('created_at') as $reply)
                                                            <div class="d-flex align-items-start gap-2 mb-2 p-2 rounded-3" style="background:rgba(45,90,39,0.05);" id="modal-comment-{{ $reply->id }}">
                                                                <div class="rounded-circle d-flex align-items-center justify-content-center text-white fw-bold flex-shrink-0" style="width:24px;height:24px;font-size:0.65rem;background:var(--btn-bg);opacity:0.8;">{{ strtoupper(substr($reply->user->name ?? '?', 0, 1)) }}</div>
                                                                <div class="flex-grow-1">
                                                                    <div class="d-flex justify-content-between align-items-center">
                                                                        <span class="fw-bold text-main" style="font-size:0.8rem;">{{ $reply->user->name ?? '?' }}</span>
                                                                        <div class="d-flex align-items-center gap-2">
                                                                            <span class="opacity-50" style="font-size:0.7rem;">{{ $reply->created_at->diffForHumans() }}</span>
                                                                            @if(Auth::check() && Auth::id() === $reply->user_id)
                                                                                <form action="{{ route('comments.destroy', $reply->id) }}" method="POST" class="d-inline" onsubmit="return confirm('{{ __('Supprimer ?') }}')">@csrf @method('DELETE')<button type="submit" class="btn btn-link text-danger p-0 border-0" style="font-size:0.7rem;"><i class="fas fa-trash-alt"></i></button></form>
                                                                            @endif
                                                                        </div>
                                                                    </div>
                                                                    <p class="mb-0 text-main opacity-75" style="font-size:0.8rem;">{{ $reply->body }}</p>
                                                                </div>
                                                            </div>
                                                        @endforeach
                                                        @if($comment->replies->count() > 2)</div>@endif
                                                    @endif
                                                </div>
                                            </div>
                                        @empty
                            ...
                            <div class="flex-grow-1">
                                <div class="d-flex justify-content-between align-items-center">
                                    ...
                                    <div class="d-flex align-items-center gap-2">
                                        ...
                                        ${c.can_delete ? `
                                            <form action="${c.delete_url}" method="POST" class="d-inline" onsubmit="return confirm('{{ __('Supprimer ?') }}')">
